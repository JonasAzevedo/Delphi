...
procedure Register;
begin
  RegisterComponents('Misc Units', [TMenuDesigner, TPopupMenuDesigner]);
end;
...
var
  idx : Integer;
begin
  if Assigned (afterItem.Parent) then
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      w0 := 30 + menuLeftMargin + menuRightMargin;
...
begin
  ...
  if Assigned (fOnSelectedItemChange) and not (csDestroying in ComponentState) then
...
begin
  ...
  if TDesignerMenuItem (Item).Selected then     // Get correct brush color...
...
var
  idx : Integer;
begin
  if Assigned (beforeItem.Parent) then
  begin
