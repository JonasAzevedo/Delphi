...
uses
  ...
  ComCtrls, ToolWin, unObjDescricaoProdutos;

  type
    TDescricaoProdutos = record
      codigo: Integer;
      nome: string;
      valor: double;
    end;
...
type
  TfrmCadDescricaoProdutos = class(TfrmCadastroBasico)
  private
    FoDescricaoProdutos: TObjDescricaoProdutos;
...
uses unConstantes, unPesquisandoDescricaoProdutos, unDM,
  {unVariaveis, unSelecionaRegistroBairro, unDicas,} unPrincipal;


{$R *.dfm}

{ TfrmCadDescricaoProdutos }

(* procedimentos de controle *)

procedure TfrmCadDescricaoProdutos.iniciarTela(poForm: TForm);
begin
  inherited;
  Application.CreateForm(TfrmPesquisandoRegistroDescricaoProdutos, Self.FoPesquisa);
  //FoPesquisa.iniciarTela(frmCadBairro);
  FoPesquisa.iniciarTela(Self);
  FoPesquisa.passarParametro(sPRM_AJUSTAR_LARGURA_GRIDE,null);
  if(Assigned(Self.FoPesquisa))then
    Self.FoPesquisa.Hide;
end;

procedure TfrmCadDescricaoProdutos.passarParametro(pTipo:String; pValores:OleVariant);
var
