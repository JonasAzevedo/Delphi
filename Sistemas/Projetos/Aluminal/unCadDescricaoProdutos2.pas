...
begin
  //primeiro deleta todas as descrições dos produtos.
  try
    ...
    DM.ZConnection.Commit;
      ...
      MessageDlg('Descrição de Produtos não pode ser salvo.', mtInformation, [mbOK], 0);
      DM.ZConnection.Rollback;
      Exit;
    end;

  //salva as descrições dos produtos.
  try
    DM.ZConnection.StartTransaction;
...
procedure TfrmCadDescricaoProdutos2.btnDeletarDescricaoClick(
  Sender: TObject);
begin
  if (MessageDlg('Deseja deletar a descrição do produto ' + cdsDescricaoProdutos.FieldByName('nome').AsString + ' ?',
...
begin
  ...
  begin
    if (MessageDlg('Deseja mesmo sair do cadastro de Descrição de Produtos ?', mtConfirmation, [mbYes,mbNo],0)=mrNo)then
      Action := caNone;
  end;
end;
