...
type
  ...
  private
    ...
    procedure   EndObj;
    {@method BeginShortObj - Inicia a escrita do objeto light indicado por aIndex. Se aIndex não for informado,
    neste momento um novo objeto será criado e seu índice retornado pela função.:/}
    function    BeginShortObj(aIndex:integer=0):integer;
    {@method EndShortObj - Finaliza a escrita de um objeto light qualquer. Não é necessário indicar o índice,
    uma vez que o marcador de fim de objeto não requer.:/}
...
procedure TRLPDFFilter.FontSizeBitmapNeeded;
begin
...
begin
  ...
  with fFontSizeBitmap.Canvas do
  begin
...
initialization
  ...
  begin
    ...
    Author  :='';
    KeyWords:='';
    Creator :=CS_ProductTitleStr+' v'+IntToStr(CommercialVersion)+'.'+IntToStr(ReleaseVersion)+' \251 '+CS_CopyrightStr;
    Producer:='';
  end;
