...
var
  PaperInfoArray:array[TRLPaperSize] of TRLPaperInfo;

function PaperSizeBySize(aPaperWidth,aPaperHeight:double):TRLPaperSize;
                          
procedure DetectPaperSize(aPaperWidth,aPaperHeight:double; aOrientationLandscape:boolean;
  aForceEmulation:boolean; var aResultPaperSize:TRLSystemPaperType;
  var aResultPaperWidth,aResultPaperHeight:double; var aResultOrientation:TRLSystemOrientation);
...
var
  ...
  begin
    Result:=Abs(aPaperWidth-PaperInfoArray[aPaperSize].Width)+
      Abs(aPaperHeight-PaperInfoArray[aPaperSize].Height);
  end;
begin
  if (aPaperWidth<0) or (aPaperHeight<0) then
    Result:=fpDefault
  else
  begin
    ...
    for PaperSize:=Low(TRLPaperSize) to High(TRLPaperSize) do
      if (PaperSize<>fpCustom) and not PaperInfoArray[PaperSize].Emulated then
      begin
...
begin
  ...
  begin
    aResultPaperSize  :=PaperInfoArray[PaperSize].SystemCode;
    aResultPaperWidth :=PaperInfoArray[PaperSize].Width;
    aResultPaperHeight:=PaperInfoArray[PaperSize].Height;
  end;
{$ifndef LINUX}
...
begin
  with PaperInfoArray[aPaperSize] do
  begin
    Width      :=aWidth;
    Height     :=aHeight;
...
initialization
  ...
  SetPaperInfo(fpDefault,                    -1,    -1,'Printer Default');
...
{$ifdef LINUX}
  ...
  SetPaperEqv(fpDefault,Succ(psNPageSize));
{$endif}
