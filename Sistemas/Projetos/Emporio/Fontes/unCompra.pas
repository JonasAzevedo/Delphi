...
begin
  ...
  if (cbBxPago.ItemIndex = 0) then
    ...
    DM.qryCompra.Close;
    DM.qryCompra.SQL.Clear;
    DM.qryCompra.SQL.Add('SELECT * FROM Compra ORDER BY codigo');
    DM.qryCompra.Prepared := true;
    DM.qryCompra.Open;
    DM.cdsCompra.Refresh;
    codCompra := DM.cdsCompraCODIGO.AsInteger;
    ...
    try
      ...
      DM.qrySaida.ParamByName('codCom').AsInteger := codCompra;
      DM.qrySaida.ParamByName('dat').AsDate := StrToDate(mEdData.Text);
      DM.qrySaida.ParamByName('val').AsFloat := StrToFloat(edValorTotal.Text);
