object frmSelecionaEntrada: TfrmSelecionaEntrada
  Left = 376
  Top = 231
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'EMP'#211'RIO --> Selecionando Entradas Financeiras'
  ClientHeight = 138
  ClientWidth = 492
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object StatusBar: TStatusBar
    Left = 0
    Top = 119
    Width = 492
    Height = 19
    Panels = <
      item
        Width = 350
      end
      item
        Width = 50
      end>
  end
  object GroupBox1: TGroupBox
    Left = 0
    Top = 0
    Width = 492
    Height = 117
    Align = alTop
    Caption = 'Selecionando Entradas'
    TabOrder = 1
    object Label1: TLabel
      Left = 10
      Top = 30
      Width = 34
      Height = 13
      Caption = 'Venda:'
    end
    object Label2: TLabel
      Left = 10
      Top = 60
      Width = 35
      Height = 13
      Caption = 'Cliente:'
    end
    object Label3: TLabel
      Left = 10
      Top = 90
      Width = 26
      Height = 13
      Caption = 'Data:'
    end
    object Label4: TLabel
      Left = 80
      Top = 90
      Width = 24
      Height = 13
      Caption = 'entre'
    end
    object Label5: TLabel
      Left = 216
      Top = 90
      Width = 6
      Height = 13
      Caption = 'e'
    end
    object edVenda: TEdit
      Left = 80
      Top = 24
      Width = 209
      Height = 21
      Hint = 'Pesquisar por Venda da Entrada'
      Enabled = False
      TabOrder = 0
      Text = 'edVenda'
    end
    object edCliente: TEdit
      Left = 80
      Top = 54
      Width = 209
      Height = 21
      Hint = 'Pesquisar por Cliente da Entrada'
      Enabled = False
      TabOrder = 2
      Text = 'edCliente'
    end
    object btBtnSelecionaVenda: TBitBtn
      Left = 288
      Top = 22
      Width = 33
      Height = 25
      Hint = 'Seleciona Venda da Entrada'
      Caption = '...'
      TabOrder = 1
      OnClick = btBtnSelecionaVendaClick
    end
    object btBtnSelecionaCliente: TBitBtn
      Left = 288
      Top = 52
      Width = 33
      Height = 25
      Hint = 'Seleciona Cliente da Entrada'
      Caption = '...'
      TabOrder = 3
      OnClick = btBtnSelecionaClienteClick
    end
    object mEdDataInicial: TMaskEdit
      Left = 118
      Top = 84
      Width = 78
      Height = 21
      Hint = 'Data Inicial da Entrada'
      CharCase = ecUpperCase
      EditMask = '00/00/0000;1;_'
      MaxLength = 10
      TabOrder = 4
      Text = '  /  /    '
    end
    object mEdDataFinal: TMaskEdit
      Left = 243
      Top = 84
      Width = 78
      Height = 21
      Hint = 'Data Final da Entrada'
      CharCase = ecUpperCase
      EditMask = '00/00/0000;1;_'
      MaxLength = 10
      TabOrder = 5
      Text = '  /  /    '
    end
    object btBtnGerarRelatorio: TBitBtn
      Left = 328
      Top = 74
      Width = 153
      Height = 33
      Hint = 'Gera Relat'#243'rio das Entradas'
      Caption = 'GERAR RELAT'#211'RIO'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 6
      OnClick = btBtnGerarRelatorioClick
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    end
  end
  object ApplicationEvents: TApplicationEvents
    OnHint = ApplicationEventsHint
    Left = 328
    Top = 20
  end
  object qryEntrada: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      
        'SELECT v.codigo AS CodigoVenda,e.data,e.valor_bruto,e.valor_liqu' +
        'ido,c.nome AS Cliente'
      'FROM venda v, entrada e, cliente c'
      'WHERE v.codigo = e.cod_venda'
      'AND v.cod_cliente = c.codigo'
      'ORDER BY v.codigo')
    SQLConnection = DM.SQLConnection
    Left = 368
    Top = 22
    object qryEntradaCODIGOVENDA: TIntegerField
      FieldName = 'CODIGOVENDA'
      Required = True
    end
    object qryEntradaCLIENTE: TStringField
      FieldName = 'CLIENTE'
      Size = 80
    end
    object qryEntradaDATA: TDateField
      FieldName = 'DATA'
    end
    object qryEntradaVALOR_BRUTO: TFMTBCDField
      FieldName = 'VALOR_BRUTO'
      Precision = 15
      Size = 2
    end
    object qryEntradaVALOR_LIQUIDO: TFMTBCDField
      FieldName = 'VALOR_LIQUIDO'
      Precision = 15
      Size = 2
    end
  end
  object dspEntrada: TDataSetProvider
    DataSet = qryEntrada
    Left = 398
    Top = 22
  end
  object cdsEntrada: TClientDataSet
    Active = True
    Aggregates = <>
    Params = <>
    ProviderName = 'dspEntrada'
    Left = 428
    Top = 22
    object cdsEntradaCODIGOVENDA: TIntegerField
      FieldName = 'CODIGOVENDA'
      Required = True
    end
    object cdsEntradaDATA: TDateField
      FieldName = 'DATA'
    end
    object cdsEntradaVALOR_BRUTO: TFMTBCDField
      FieldName = 'VALOR_BRUTO'
      DisplayFormat = '#0.00'
      Precision = 15
      Size = 2
    end
    object cdsEntradaVALOR_LIQUIDO: TFMTBCDField
      FieldName = 'VALOR_LIQUIDO'
      DisplayFormat = '#0.00'
      Precision = 15
      Size = 2
    end
    object cdsEntradaCLIENTE: TStringField
      FieldName = 'CLIENTE'
      Size = 80
    end
  end
  object dsEntrada: TDataSource
    DataSet = cdsEntrada
    Left = 456
    Top = 22
  end
  object qrySoma: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      
        'SELECT SUM(e.valor_bruto) AS ValorBruto,SUM(e.valor_liquido) AS ' +
        'ValorLiquido'
      'FROM venda v, entrada e, cliente c'
      'WHERE v.codigo = e.cod_venda'
      'AND v.cod_cliente = c.codigo')
    SQLConnection = DM.SQLConnection
    Left = 368
    Top = 56
    object qrySomaVALORBRUTO: TFMTBCDField
      FieldName = 'VALORBRUTO'
      Precision = 15
      Size = 2
    end
    object qrySomaVALORLIQUIDO: TFMTBCDField
      FieldName = 'VALORLIQUIDO'
      Precision = 15
      Size = 2
    end
  end
  object dspSoma: TDataSetProvider
    DataSet = qrySoma
    Left = 396
    Top = 56
  end
  object cdsSoma: TClientDataSet
    Active = True
    Aggregates = <>
    Params = <>
    ProviderName = 'dspSoma'
    Left = 426
    Top = 56
    object cdsSomaVALORBRUTO: TFMTBCDField
      FieldName = 'VALORBRUTO'
      DisplayFormat = '#0.00'
      Precision = 15
      Size = 2
    end
    object cdsSomaVALORLIQUIDO: TFMTBCDField
      FieldName = 'VALORLIQUIDO'
      DisplayFormat = '#0.00'
      Precision = 15
      Size = 2
    end
  end
  object dsSoma: TDataSource
    DataSet = cdsSoma
    Left = 455
    Top = 56
  end
end
