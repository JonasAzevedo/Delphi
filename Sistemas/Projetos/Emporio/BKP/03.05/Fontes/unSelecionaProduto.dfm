object frmSelecionaProduto: TfrmSelecionaProduto
  Left = 501
  Top = 129
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'EMP'#211'RIO  -->  Selecionando Produto'
  ClientHeight = 422
  ClientWidth = 543
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object grpBxProdutos: TGroupBox
    Left = 0
    Top = 81
    Width = 543
    Height = 249
    Align = alTop
    Caption = 'Produtos'
    TabOrder = 0
    object DBGridProdutos: TDBGrid
      Left = 2
      Top = 15
      Width = 539
      Height = 232
      Hint = 'Produtos Cadastrados'
      Align = alClient
      DataSource = DM.dsProduto
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
      PopupMenu = PopupMenu
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      OnCellClick = DBGridProdutosCellClick
      OnDblClick = btBtnSelecionarProdutoClick
      OnExit = DBGridProdutosExit
      Columns = <
        item
          Expanded = False
          FieldName = 'NOME'
          Title.Caption = 'Nome'
          Width = 397
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ESTOQUE'
          Title.Caption = 'Estoque'
          Visible = True
        end>
    end
  end
  object btBtnNovoProduto: TBitBtn
    Left = 386
    Top = 368
    Width = 149
    Height = 25
    Hint = 'Abrir Cadastro de um Novo Produto'
    Caption = 'Cadastrar Novo Produto'
    TabOrder = 1
    OnClick = btBtnNovoProdutoClick
  end
  object grpBxPesquisa: TGroupBox
    Left = 0
    Top = 0
    Width = 543
    Height = 57
    Align = alTop
    Caption = 'Pesquisar'
    TabOrder = 2
    object rdBtnCodigo: TRadioButton
      Left = 8
      Top = 20
      Width = 57
      Height = 17
      Hint = 'Pesquisar por C'#243'digo do Produto'
      Caption = 'C'#243'digo'
      TabOrder = 0
      OnClick = rdBtnCodigoClick
    end
    object rdBtnNome: TRadioButton
      Left = 8
      Top = 35
      Width = 83
      Height = 17
      Hint = 'Pesquisar por Nome do Produto'
      Caption = 'Nome'
      TabOrder = 1
      OnClick = rdBtnNomeClick
    end
    object edPesquisa: TEdit
      Left = 96
      Top = 24
      Width = 281
      Height = 21
      Hint = 'Valor a ser Pesquisado'
      CharCase = ecUpperCase
      TabOrder = 2
      OnChange = edPesquisaChange
      OnKeyPress = edPesquisaKeyPress
    end
    object btBtnPesquisar: TBitBtn
      Left = 384
      Top = 17
      Width = 57
      Height = 33
      Hint = 'Pesquisar por Produto'
      BiDiMode = bdLeftToRight
      Caption = 'OK'
      Enabled = False
      ParentBiDiMode = False
      TabOrder = 3
      OnClick = btBtnPesquisarClick
      Glyph.Data = {
        
        95CB1193CB1092CA0D8FC7FF00FFFF00FFFF00FFFF00FFFF00FF}
    end
  end
  object pnlDivisao: TPanel
    Left = 0
    Top = 57
    Width = 543
    Height = 24
    Align = alTop
    BevelOuter = bvNone
    TabOrder = 3
  end
  object StatusBar: TStatusBar
    Left = 0
    Top = 403
    Width = 543
    Height = 19
    Panels = <
      item
        Width = 360
      end
      item
        Width = 50
      end>
  end
  object btBtnSelecionarProduto: TBitBtn
    Left = 8
    Top = 351
    Width = 201
    Height = 34
    Hint = 'Selecionar Produto para Compra Atual'
    Caption = 'Selecionar Produto da Compra'
    TabOrder = 5
    OnClick = btBtnSelecionarProdutoClick
  end
  object btBtnMostrarTodosProdutos: TBitBtn
    Left = 385
    Top = 341
    Width = 150
    Height = 25
    Hint = 'Mostrar Todos os Produtos Cadastrados'
    Caption = 'Mostrar Todos Produtos'
    TabOrder = 6
    OnClick = btBtnMostrarTodosProdutosClick
  end
  object ApplicationEvents: TApplicationEvents
    OnHint = ApplicationEventsHint
    Left = 488
    Top = 20
  end
  object PopupMenu: TPopupMenu
    Left = 455
    Top = 21
    object ppMnMostrarTodosFornecedores: TMenuItem
      Caption = 'Mostrar Todos Produtos'
      Hint = 'Mostrar Todos os Produtos Cadastrados'
      OnClick = ppMnMostrarTodosFornecedoresClick
    end
  end
end
