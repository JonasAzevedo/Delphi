object frmSelecionaTipoProduto: TfrmSelecionaTipoProduto
  Left = 675
  Top = 246
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'NOIVA LINDA --> Selecionando Tipo Produto'
  ClientHeight = 416
  ClientWidth = 589
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object grpBxTiposProduto: TGroupBox
    Left = 0
    Top = 81
    Width = 589
    Height = 249
    Align = alTop
    Caption = 'Tipos de Produto'
    TabOrder = 0
    object DBGridTiposProduto: TDBGrid
      Left = 2
      Top = 15
      Width = 585
      Height = 232
      Hint = 'Tipos de Produto Cadastrados'
      Align = alClient
      DataSource = DM.dsTipoProduto
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
      PopupMenu = PopupMenu
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      OnCellClick = DBGridTiposProdutoCellClick
      OnDblClick = DBGridTiposProdutoDblClick
      OnExit = DBGridTiposProdutoExit
      Columns = <
        item
          Expanded = False
          FieldName = 'CODIGO'
          Title.Caption = 'C'#243'digo'
          Width = 60
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'TIPO'
          Title.Caption = 'Tipo'
          Width = 160
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DESCRICAO'
          Title.Caption = 'Descri'#231#227'o'
          Width = 320
          Visible = True
        end>
    end
  end
  object btBtnNovoTipoProduto: TBitBtn
    Left = 7
    Top = 350
    Width = 180
    Height = 34
    Hint = 'Abrir Cadastro de um Novo Tipo de Produto'
    Caption = 'Cadastrar Novo Tipo de Produto'
    TabOrder = 1
    OnClick = btBtnNovoTipoProdutoClick
  end
  object grpBxPesquisa: TGroupBox
    Left = 0
    Top = 0
    Width = 589
    Height = 57
    Align = alTop
    Caption = 'Pesquisar'
    TabOrder = 2
    object rdBtnCodigo: TRadioButton
      Left = 8
      Top = 20
      Width = 57
      Height = 17
      Hint = 'Pesquisar por C'#243'digo do Tipo de Produto'
      Caption = 'C'#243'digo'
      TabOrder = 0
      OnClick = rdBtnCodigoClick
    end
    object rdBtnNome: TRadioButton
      Left = 8
      Top = 35
      Width = 53
      Height = 17
      Hint = 'Pesquisar por Nome do Tipo de Produto'
      Caption = 'Nome'
      TabOrder = 1
      OnClick = rdBtnNomeClick
    end
    object edPesquisa: TEdit
      Left = 96
      Top = 24
      Width = 417
      Height = 21
      Hint = 'Valor a ser Pesquisado'
      CharCase = ecUpperCase
      TabOrder = 2
      OnChange = edPesquisaChange
      OnEnter = edPesquisaEnter
      OnExit = edPesquisaExit
      OnKeyDown = edPesquisaKeyDown
      OnKeyPress = edPesquisaKeyPress
    end
    object btBtnPesquisar: TBitBtn
      Left = 520
      Top = 17
      Width = 57
      Height = 33
      Hint = 'Pesquisar por Tipo de Produto'
      BiDiMode = bdLeftToRight
      Caption = 'OK'
      Enabled = False
      ParentBiDiMode = False
      TabOrder = 3
      OnClick = btBtnPesquisarClick
      Glyph.Data = {
        
        95CB1193CB1092CA0D8FC7FF00FFFF00FFFF00FFFF00FFFF00FF}
    end
  end
  object pnlDivisao: TPanel
    Left = 0
    Top = 57
    Width = 589
    Height = 24
    Align = alTop
    BevelOuter = bvNone
    TabOrder = 3
  end
  object StatusBar: TStatusBar
    Left = 0
    Top = 397
    Width = 589
    Height = 19
    Panels = <
      item
        Width = 380
      end
      item
        Width = 50
      end>
  end
  object btBtnSelecionarTipoProduto: TBitBtn
    Left = 400
    Top = 350
    Width = 180
    Height = 34
    Hint = 'Selecionar Tipo de Produto para Opera'#231#227'o Atual'
    Caption = 'Selecionar Tipo de Produto'
    TabOrder = 5
    OnClick = DBGridTiposProdutoDblClick
  end
  object ApplicationEvents: TApplicationEvents
    OnHint = ApplicationEventsHint
    Left = 488
    Top = 20
  end
  object PopupMenu: TPopupMenu
    Left = 455
    Top = 22
    object ppMnMostrarTodosTiposProduto: TMenuItem
      Caption = 'Mostrar Todos Tipos de Produto'
      Hint = 'Mostrar Todos os Tipos de Produto Cadastrados'
      OnClick = ppMnMostrarTodosTiposProdutoClick
    end
  end
end
