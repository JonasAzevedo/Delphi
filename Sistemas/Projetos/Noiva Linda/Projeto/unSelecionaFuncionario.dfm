object frmSelecionaFuncionario: TfrmSelecionaFuncionario
  Left = 388
  Top = 192
  Width = 597
  Height = 388
  BorderIcons = [biSystemMenu, biMinimize]
  Caption = 'NOIVA LINDA --> Selecionando Funcion'#225'rio'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object grpBxFuncionarios: TGroupBox
    Left = 0
    Top = 73
    Width = 589
    Height = 249
    Align = alTop
    Caption = 'Funcion'#225'rios'
    TabOrder = 0
    object DBGridFuncionarios: TDBGrid
      Left = 2
      Top = 15
      Width = 585
      Height = 232
      Hint = 'Funcion'#225'rios Cadastrados'
      Align = alClient
      DataSource = DM.dsUsuario
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
      PopupMenu = PopupMenu
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      OnCellClick = DBGridFuncionariosCellClick
      OnDblClick = DBGridFuncionariosDblClick
      OnExit = DBGridFuncionariosExit
      Columns = <
        item
          Expanded = False
          FieldName = 'CODIGO'
          Title.Caption = 'C'#243'digo'
          Width = 60
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NOME'
          Title.Caption = 'Nome'
          Width = 480
          Visible = True
        end>
    end
  end
  object grpBxPesquisa: TGroupBox
    Left = 0
    Top = 0
    Width = 589
    Height = 57
    Align = alTop
    Caption = 'Pesquisar'
    TabOrder = 1
    object rdBtnCodigo: TRadioButton
      Left = 8
      Top = 20
      Width = 57
      Height = 17
      Hint = 'Pesquisar por C'#243'digo do Funcion'#225'rio'
      Caption = 'C'#243'digo'
      TabOrder = 0
      OnClick = rdBtnCodigoClick
    end
    object rdBtnNome: TRadioButton
      Left = 8
      Top = 35
      Width = 53
      Height = 17
      Hint = 'Pesquisar por Nome do Funcion'#225'rio'
      Caption = 'Nome'
      TabOrder = 1
      OnClick = rdBtnNomeClick
    end
    object edPesquisa: TEdit
      Left = 96
      Top = 24
      Width = 417
      Height = 21
      Hint = 'Valor a ser Pesquisado'
      CharCase = ecUpperCase
      TabOrder = 2
      OnChange = edPesquisaChange
      OnEnter = edPesquisaEnter
      OnExit = edPesquisaExit
      OnKeyDown = edPesquisaKeyDown
      OnKeyPress = edPesquisaKeyPress
    end
    object btBtnPesquisar: TBitBtn
      Left = 520
      Top = 17
      Width = 57
      Height = 33
      Hint = 'Pesquisar por Funcion'#225'rio'
      BiDiMode = bdLeftToRight
      Caption = 'OK'
      Enabled = False
      ParentBiDiMode = False
      TabOrder = 3
      OnClick = btBtnPesquisarClick
      Glyph.Data = {
        
        95CB1193CB1092CA0D8FC7FF00FFFF00FFFF00FFFF00FFFF00FF}
    end
  end
  object pnlDivisao: TPanel
    Left = 0
    Top = 57
    Width = 589
    Height = 16
    Align = alTop
    BevelOuter = bvNone
    TabOrder = 2
  end
  object StatusBar: TStatusBar
    Left = 0
    Top = 335
    Width = 589
    Height = 19
    Panels = <
      item
        Width = 450
      end
      item
        Width = 50
      end>
  end
  object ApplicationEvents: TApplicationEvents
    OnHint = ApplicationEventsHint
    Left = 488
    Top = 20
  end
  object PopupMenu: TPopupMenu
    Left = 455
    Top = 22
    object ppMnMostrarTodosFuncionarios: TMenuItem
      Caption = 'Mostrar Todos Funcion'#225'rios'
      Hint = 'Mostrar Todos os Funcion'#225'rios Cadastrados'
      OnClick = ppMnMostrarTodosFuncionariosClick
    end
  end
end
