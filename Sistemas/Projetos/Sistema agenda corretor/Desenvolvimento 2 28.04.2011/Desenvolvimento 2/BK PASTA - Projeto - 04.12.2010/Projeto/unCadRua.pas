...
uses unConstantes, unPesquisandoRua, unDM, unSelecionaRegistroBairro,
...
begin
  ...
  //verifica se rua não está associada a um cliente
  if(bPodeDeletar)then
  begin
    DM.zQryCliente.Close;
    DM.zQryCliente.SQL.Clear;
    DM.zQryCliente.SQL.Add('SELECT * FROM cliente WHERE cod_fk_rua=:cod_fk_rua');
    DM.zQryCliente.ParamByName('cod_fk_rua').AsInteger := Self.FoRua.getCodigo;
    DM.zQryCliente.Open;
  ...
  //verifica se rua não está associada a uma pessoa_fisica
  if(bPodeDeletar)then
  begin
    DM.zQryPessoaFisica.Close;
    DM.zQryPessoaFisica.SQL.Clear;
    DM.zQryPessoaFisica.SQL.Add('SELECT * FROM pessoa_fisica WHERE cod_fk_rua=:cod_fk_rua');
    DM.zQryPessoaFisica.ParamByName('cod_fk_rua').AsInteger := Self.FoRua.getCodigo;
    DM.zQryPessoaFisica.Open;
    if(DM.zQryPessoaFisica.RecordCount <> 0)then
    begin
      MessageDlg('Rua não pode ser deletada, pois existe uma pessoa física desta rua.',
        mtInformation, [mbOK], 0);
      bPodeDeletar := False;
    end;
  end;
  ...
  begin
    ...
    DM.zQryPessoaJuridica.SQL.Add('SELECT * FROM pessoa_juridica WHERE cod_fk_rua=:cod_fk_rua');
    DM.zQryPessoaJuridica.ParamByName('cod_fk_rua').AsInteger := Self.FoRua.getCodigo;
    DM.zQryPessoaJuridica.Open;
    if(DM.zQryPessoaJuridica.RecordCount <> 0)then
    begin
      MessageDlg('Rua não pode ser deletada, pois existe uma pessoa jurídica desta rua.',
        mtInformation, [mbOK], 0);
      bPodeDeletar := False;
    end;
  end;
...
procedure TfrmCadRua.plsEdBairroClick(Sender: TObject);
begin
  inherited;
