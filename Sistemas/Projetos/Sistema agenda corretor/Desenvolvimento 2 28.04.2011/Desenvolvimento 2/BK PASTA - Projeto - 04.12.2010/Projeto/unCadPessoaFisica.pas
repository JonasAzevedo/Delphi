...
begin
  ...
  zQryDados.Close;
  zQryDados.SQL.Clear;
  zQryDados.SQL.Add('SELECT * FROM pessoa_fisica ');
  ...
  begin
    ...
    begin
      try
        ...
        zQryDados.Close;
        zQryDados.SQL.Clear;
        zQryDados.SQL.Add('UPDATE pessoa_fisica SET nome=:nom,email=:ema,cpf=:cpf, ');
    ...
    begin
      try
        ...
        zQryDados.Close;
        zQryDados.SQL.Clear;
        zQryDados.SQL.Add('INSERT INTO pessoa_fisica (nome,email,cpf,');
...
begin
  ...
  begin
    ...
    DM.zQryPessoaFisica.SQL.Add('SELECT * FROM pessoa_fisica ORDER BY codigo DESC ROWS 1');
    DM.zQryPessoaFisica.Open;
    if(DM.zQryPessoaFisica.RecordCount = 1)then
      nCodigo := DM.zQryPessoaFisicaCODIGO.AsInteger;
  ...
  begin
    DM.zQryPessoaFisica.Close;
    DM.zQryPessoaFisica.SQL.Clear;
    DM.zQryPessoaFisica.SQL.Add('SELECT * FROM pessoa_fisica ');
    DM.zQryPessoaFisica.SQL.Add('WHERE codigo=:codigo');
    DM.zQryPessoaFisica.ParamByName('codigo').AsInteger := Self.FoPessoaFisica.getCodigo;
    DM.zQryPessoaFisica.Open;
    if(DM.zQryPessoaFisica.RecordCount = 1)then
    begin
      nCodigo := DM.zQryPessoaFisicaCODIGO.AsInteger;
      sNome := DM.zQryPessoaFisicaNOME.AsString;
    end;
...
begin
  ...
  try
    ...
    zQryDados.Close;
    zQryDados.SQL.Clear;
    zQryDados.SQL.Add('DELETE FROM pessoa_fisica WHERE codigo=:cod');
    zQryDados.ParamByName('cod').AsInteger := Self.FoPessoaFisica.getCodigo();
    zQryDados.ExecSQL();
    DM.ZConnection.Commit;
    bDeletou := True;
...
begin
  ...
  zQryDados.Close;
  zQryDados.SQL.Clear;
  zQryDados.SQL.Add('SELECT * FROM pessoa_fisica WHERE nome=:nom AND cpf=:cpf ');
  ...
  begin
    ...
    begin
      ...
      begin
        zQryDados.Close;
        zQryDados.SQL.Clear;
        zQryDados.SQL.Add('SELECT * FROM pessoa_fisica WHERE cpf=:cpf ');
        //se estiver editando, pode salvar com os mesmos dados
        if(Self.FoPessoaFisica.getCodigo <> NUMERO_INDEFINIDO)then
        begin
          zQryDados.SQL.Add(' AND codigo <> :cod');
...
procedure TfrmCadPessoaFisica.editarRegistroChamadaExterna();
  ...
  begin
    ...
    zQryDados.Close;
    zQryDados.SQL.Clear;
    zQryDados.SQL.Add('SELECT * FROM pessoa_fisica WHERE codigo=:cod');
...
(*  inherited;
  ...
  begin
    ...
    begin
      sValor := plsMedCPF.Text + Key;
      sValor := UpperCase(sValor);
      Self.FoPesquisa.atualizarPesquisando('CPF', 'CPF:', sValor, cdsDados);
      Self.FoPesquisa.Show;
    end;
  end;
*)  
...
(*  inherited;
  ...
  begin
    ...
    begin
      sValor := plsEdIdentidade.Text + Key;
      sValor := UpperCase(sValor);
      Self.FoPesquisa.atualizarPesquisando('IDENTIDADE', 'Identidade:', sValor, cdsDados);
      Self.FoPesquisa.Show;
    end;
  end;
*)  
