...
begin
  ...
  zQryDados.Close;
  zQryDados.SQL.Clear;
  zQryDados.SQL.Add('SELECT * FROM pessoa_juridica ');
  ...
  begin
    ...
    begin
      try
        ...
        zQryDados.Close;
        zQryDados.SQL.Clear;
        zQryDados.SQL.Add('UPDATE pessoa_juridica SET nome=:nom,email=:ema,cnpj=:cnpj, ');
    ...
    begin
      try
        ...
        zQryDados.Close;
        zQryDados.SQL.Clear;
        zQryDados.SQL.Add('INSERT INTO pessoa_juridica (nome,email,cnpj,');
...
begin
  ...
  begin
    ...
    DM.zQryPessoaJuridica.SQL.Add('SELECT * FROM pessoa_juridica ORDER BY codigo DESC ROWS 1');
    DM.zQryPessoaJuridica.Open;
    if(DM.zQryPessoaJuridica.RecordCount = 1)then
      nCodigo := DM.zQryPessoaJuridicaCODIGO.AsInteger
    else
      nCodigo := NUMERO_INDEFINIDO;
  ...
  begin
    DM.zQryPessoaJuridica.Close;
    DM.zQryPessoaJuridica.SQL.Clear;
    DM.zQryPessoaJuridica.SQL.Add('SELECT * FROM pessoa_juridica ');
    DM.zQryPessoaJuridica.SQL.Add('WHERE codigo=:codigo');
    DM.zQryPessoaJuridica.ParamByName('codigo').AsInteger := Self.FoPessoaJuridica.getCodigo;
    DM.zQryPessoaJuridica.Open;
    if(DM.zQryPessoaJuridica.RecordCount = 1)then
    begin
      nCodigo := DM.zQryPessoaJuridicaCODIGO.AsInteger;
      sNome := DM.zQryPessoaJuridicaNOME.AsString;
    end;
...
begin
  ...
  try
    ...
    zQryDados.Close;
    zQryDados.SQL.Clear;
    zQryDados.SQL.Add('DELETE FROM pessoa_juridica WHERE codigo=:cod');
    zQryDados.ParamByName('cod').AsInteger := Self.FoPessoaJuridica.getCodigo();
    zQryDados.ExecSQL();
    DM.ZConnection.Commit;
    bDeletou := True;
...
begin
  ...
  zQryDados.Close;
  zQryDados.SQL.Clear;
  zQryDados.SQL.Add('SELECT * FROM pessoa_juridica WHERE nome=:nom AND cnpj=:cnpj ');
  ...
  begin
    ...
    begin
      ...
      begin
        zQryDados.Close;
        zQryDados.SQL.Clear;
        zQryDados.SQL.Add('SELECT * FROM pessoa_juridica WHERE cnpj=:cnpj ');
        //se estiver editando, pode salvar com os mesmos dados
        if(Self.FoPessoaJuridica.getCodigo <> NUMERO_INDEFINIDO)then
        begin
          zQryDados.SQL.Add(' AND codigo <> :cod');
...
procedure TfrmCadPessoaJuridica.editarRegistroChamadaExterna();
  ...
  begin
    ...
    zQryDados.Close;
    zQryDados.SQL.Clear;
    zQryDados.SQL.Add('SELECT * FROM pessoa_juridica WHERE codigo=:cod');
