...
type
  TfrmPesquisarImoveisInteresse = class(TfrmPesquisar)
    ...
    zQryDadosCC_RENDIMENTO: TStringField;
    ...
    cdsDadosCC_RENDIMENTO: TStringField;
...
begin
  ...
  zQryDados.SQL.Add('CAST(i.rendimento AS VARCHAR(20)) AS CC_RENDIMENTO,');
...
begin
  ...
  begin
    ...
    begin
      ...
      sValorMonetario :=  cdsDadosCC_RENDIMENTO.AsString;
      sValorMonetario := StringReplace(sValorMonetario, '.', ',', [rfReplaceAll, rfIgnoreCase]);
      if(sValorMonetario <> STRING_INDEFINIDO)then
        cdsDadosCC_RENDIMENTO.AsString := Self.FoFuncoes.formatarValorMonetario(sValorMonetario);
