...
type
  ...
  protected
    ...
    procedure MostrarHintCampos(psPesquisando, psNovoEditar:String); dynamic;
  end;
...
begin
  if (Self.FbPodePesquisar)and(not(Self.FbPesquisouKeyEnter)) then
    ...
    StatusBar.Panels[0].Text := 'Digite o ' +psNovoEditar+ ' que está cadastrando'
  else if(Self.FiFuncaoRegistro = FUNCAO_REGISTRO_EDITAR)then
    StatusBar.Panels[0].Text := 'Digite o ' +psNovoEditar+ ' que está editando';
end;
